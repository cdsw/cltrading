...
save_file = "./models/B5C/a3crestrain-{}.pkl"
...
class BTCTradingEnvCont(gym.Env):
    ...
    def _take_action(self, action):
        ...
        try:
          ...
          amount = min((action % 6) / 5 + random.random() / 5, 1)
        except:
          print(action)
...
class Agent(mp.Process):
    def __init__(self, global_actor_critic, optimizer, input_dims, n_actions, 
        ...
        self.env = BTCTradingEnvCont(df, 'a3c')
...
class Tester(mp.Process):
    def __init__(self, global_actor_critic, input_dims, n_actions, gamma, data):
        ...
        self.local_actor_critic = ActorCritic(input_dims, n_actions, gamma)
        self.global_actor_critic = global_actor_critic
        self.env = BTCTradingEnvCont(data, 'a3c') #CHANGE THIS
...
if __name__ == '__main__':
    lr = 1e-4
    n_actions = 18 #0~5: buy, 6~11: sell, 12~17: hold
    input_dims = [6]
    ...
    for addr in data_addrs:
      ...
      pkl.dump(results_, open(f'./models/B5C/res-a3c-c{addr}', 'ab'))
